...
class ParametricContinuousParkour(gym.Env, EzPickle):
    ...
    def __init__(self, agent_body_type, CPPN_weights_path = None, input_CPPN_dim=3, terrain_cppn_scale=10,
        ...
        high = np.array([np.inf] * (agent_state_size +
                                    ...
                                    NB_LIDAR*2))# + 1))  # lidars infos + if creeper for each lidar
    ...
    def step(self, action):
        ...
        # Penalize the agent for not being in its preferred environment
        # if self.agent_body.body_type == BodyTypesEnum.SWIMMER and not is_under_water:
        #     reward -= 0.1
        # elif self.agent_body.body_type in [BodyTypesEnum.WALKER, BodyTypesEnum.CLIMBER] and is_under_water:
        #     reward -= 0.1
    ...
    def _generate_terrain(self):
        ...
        # Water
        # Calculate air area
        # air_max_distance = max(ceiling_y) - min(ground_y)
        # water_y = min(ground_y) + self.water_level * air_max_distance
